...
###
### Description:
###  Rnw File for lecture 01
###
### History:
###  -- 2020-06-10 file created as part of SmitSjuk
###  -- 2020-02-21 modified as part of STA427
######################################################################
...
## ----echo=FALSE---------------------------------------------------------------
R0.grid <- seq(1,10,length=91)
#Given R0 solve for f (final size)
solve.f <- function(R0) {
  uniroot( function(f) 1-f-exp(-R0*f), lower=1e-12,upper=1-1e-12)$root
}
f.grid <- sapply(R0.grid, solve.f)
#F for the examples
#sapply(R0, solve.f)


## ----warning=FALSE------------------------------------------------------------
pal <- brewer.pal(n=2,"Set1")
par(mar=c(4.5,4.5,1,2))
#Compute maximal value of i(t)
i.max <- function(R0) {
  1 - 1/R0*(1+log(R0))
}
i.max.grid <- sapply(R0.grid, i.max)
#
plot(R0.grid, f.grid, type="l",xlim=c(0,max(R0.grid)),xlab=expression(R[0]),ylab="Fraction infected",lwd=3,col=pal[1])
lines(R0.grid, i.max.grid,lty=2,lwd=2,col=pal[2])
legend(x="bottomright",c("f",expression(i[max])),col=pal[1:2],lty=1:2,lwd=3)
